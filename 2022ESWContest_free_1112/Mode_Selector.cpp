...
void Jetson1Callback(const std_msgs::String::ConstPtr& msg)
{
...
int main(int argc, char** argv) {
    ...
    ros::Publisher type_pub = n.advertise<std_msgs::String>("type", 1000);
    ros::Subscriber Jetson1_sub = n.subscribe("Jetson1", 1000, Jetson1Callback);
    ...
    while (ros::ok())
    {
        if (mode > 0) {
            ...
            if (mode == 1) {
                ...
                ss1 << type;
                msg.data = ss1.str();
                ROS_INFO("nowType : %s", msg.data.c_str());
