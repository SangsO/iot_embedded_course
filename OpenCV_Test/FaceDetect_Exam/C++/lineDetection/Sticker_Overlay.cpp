...
StickerOverlay::StickerOverlay(cv::Mat *frame, std::vector<std::vector<cv::Point2f>> *landmarks, int pn)
...
void StickerOverlay::set_object_center()
{
...
void StickerOverlay::contourse_filter(int B_height, int B_width)
{
...
void StickerOverlay::set_item_nobackgnd_data()
{
...
GlassSticker::GlassSticker(cv::Mat *frame, std::vector<std::vector<cv::Point2f>> *landmarks, int pn)
	: StickerOverlay(frame, landmarks, pn)
{
...
GlassSticker::~GlassSticker()
{}

void GlassSticker::set_boundary()
{
...
LipSticker::LipSticker(cv::Mat *frame, std::vector<std::vector<cv::Point2f>> *landmarks, int pn)
	: StickerOverlay(frame, landmarks, pn)
{
	//img_resize = img.clone();
	img_resize = IO::item_IO_data[1].second.clone();
...
LipSticker::~LipSticker()
{}

void LipSticker::set_boundary()
{
	object_left = std::make_pair((*landmarks)[pn][48].x, (*landmarks)[pn][48].y);
	object_right = std::make_pair((*landmarks)[pn][54].x, (*landmarks)[pn][54].y);
	object_up = std::make_pair((*landmarks)[pn][51].x, (*landmarks)[pn][51].y);
	object_down = std::make_pair((*landmarks)[pn][57].x, (*landmarks)[pn][57].y);
}