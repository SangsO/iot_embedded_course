...
static int led_init(void) //GPIO pin (LED) 초기화 함수
{
	int ret = 0;
	int i;
	...
	for (i = 0; i < ARRAY_SIZE(led); i++) {
		...
		if(ret<0){
			...
			gpio_direction_output(led[i], 0);  // 0:led off
											   // GPIO 출력 모드 변경
											   // gpio_direction_output(gp_nr, init_val);
											   //init_val 는 초기값
	}
	return ret;
}

static int key_init(void)						//GPIO pin (스위치) 초기화
{
	int ret = 0;
	int i;
...
static void led_write(char data)
{
	int i;
	for(i = 0; i < ARRAY_SIZE(led); i++){
		gpio_set_value(led[i], (data >> i ) & 0x01);	//GPIO 핀[1]을 1로 올려 준다
	}
#if DEBUG
	printk("#### %s, data = %d\n", __FUNCTION__, data);
#endif
...
//	char temp;
	for(i=0;i<ARRAY_SIZE(key);i++)
	{
		if(gpio_get_value(key[i]))					//GPIO 핀을 읽는다
		{
			data = i+1;
			break;
		}
...
static ssize_t ledkey_read(struct file *filp, char *buf, size_t count, loff_t *f_pos)//과제(추가된부분)
...
//사용자 영역인 buf에서 count바이트만큼 읽은 후 device의 f_pos 위치로 저장
//struct file *flip 	읽기와 쓰기에 전달되는 flip은 device file이 어떤 형식으로 열렸는가에 대한 정보를 저장
//loff_t f_ops			f_pos 필드 변수에는 현재의 읽기 / 쓰기 위치를 저장한다
{
	char kbuf;			//kernel buff
	int ret;
//	get_user(kbuf,buf); //kbuf 변수에 buf의 user memory 값을 대입
	ret = copy_from_user(&kbuf,buf,count);
	//kernel 영역과 user 영역은 서로 접근하지 못하는 메모리의 영역이기 때문에 pointer를 이용하지 못함
	//지금과 같이 dev 파일과 app파일로 나누는 이유이다?!
	//데이터를 전달하는 copy_from_user 함수를 이용
	//user 영역의 buf값을 kernel 영역의 kbuf에 전달
...
{	//kernel 영역에서의 ioctl 함수
	//inode와 flip pointer는 응용 프로그램의 file descriptor와 일치하는 인자이다
	...
	//[16:23]매직넘버
	//[24:31]구분 번호
	//arg는 명령 실행의 결과 데이터가 전달되는 unsigned long 형의 정수(또는 포인터)
	...
	*/
	...
	switch( cmd )		//일반적으로 I/O 컨트롤에 관한 작업을 수행하는 함수
						//대부분의 ioctl 메소드 구현은 cmd인자값에 따라 올바른 동작을 선택하는 switch문으로 구성
	{
		/* KEY, LED 초기화 */
...
struct file_operations ledkey_fops =
{
	...
	.unlocked_ioctl = ledkey_ioctl, //모든 cpu에서 lock을 걸던 것을 개별적인 lock을 걸 수 있도록 ioctl에서 바꾼 함수
         							//함수에 진입 시 kernel에 대한 lock이 처리되어지지 않기 때문에 함수 내에서 별도의 동기화 기법을 사용해야 함.
									//이 함수를 호출하면 된다
	.open     = ledkey_open,		//device driver가 처음 열렸을 때 하드웨어 초기화
